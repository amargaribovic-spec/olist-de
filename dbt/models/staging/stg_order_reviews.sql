...
select
    ({{ dbt_utils.generate_surrogate_key(['review_id', 'order_id']) }})::varchar as review_sk,
    review_id::varchar as review_id,
    order_id::varchar as order_id,
    nullif(review_score, '')::int as review_score,
    ({{ clean_text('review_comment_title') }})::varchar as review_comment_title,
    ({{ clean_text('review_comment_message') }})::varchar as review_comment_message,
